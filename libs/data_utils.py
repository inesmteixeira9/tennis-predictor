...
def upload_csvfile(csv_file, local_path: str) -> pd.DataFrame:
    ...
    try:
        ...
        logging.error(f'Error reading raw_data as df using read_raw_data: {e}')
...
def read_data(path: str) -> pd.DataFrame:
    ...
    Args:
        path (str): The path to the directory containing CSV and pickle files.
    ...
    try:
        if path.endswith('.csv'):
            ...
            logging.info(f"Reading all files in {path}...")
            data_files = glob.glob(os.path.join(path, '*.csv')) + glob.glob(os.path.join(path, '*.pkl'))

            if not data_files:
                raise FileNotFoundError(f"No CSV or pickle files found in {path}")
        ...
        logging.info(f"-> Shape of the dataset: {dataset.shape}")
        return dataset

    except FileNotFoundError:
        raise FileNotFoundError(f"The specified data_path '{path}' does not exist.")
...
def read_csv(file_path: str) -> pd.DataFrame:
    ...
    try:
        ...
        logging.error(f"Error reading empty data from file {file_path}: {empty_data_error}")
    except Exception as e:
...
def read_pkl(file_path: str) -> pd.DataFrame:
    ...
    try:
        ...
        logging.error(f"Error unpickling file {file_path}: {unpickling_error}")
    except Exception as e:
        logging.error(f"Error reading pickle file {file_path}: {str(e)}")
...
def format_data(df: pd.DataFrame, schema: dict) -> pd.DataFrame:
    ...
    try:
        ...
        if not object_cols.empty:
            logging.warning(f"The following columns are of type object and were not formatted: {', '.join(object_cols)}")
        else:
            logging.info('-> Data formatted according to the imported schema...')
...
def split_data(X, y, train_size, test_size, cv=False):
    ...
    if cv == True: 
        ...
        logging.info(f"Splitting data randomly into a training ({train_size * 100}%) and a testing ({test_size * 100}%) datasets.")
        ...
        logging.info(f"Splitting data randomly into a training ({train_size * 100}%), a validation ({round(1 - train_size - test_size, 2) * 100}%) and a testing ({test_size * 100}%) datasets.")
...
def save_data(data: pd.DataFrame, file_name: str, path: str):
    ...
    try:
        ...
        if file_name.endswith('.csv'):
            ...
            logging.error(f"Unrecognized file type for {file_name}.")
            return

        logging.info(f"Saved {file_name} to {path}")
    except Exception as e:
